...
# -------- Level 1 --------
user_Choice = input("What would you like to do? 1. Look at available books 2. Search for a book 3. Checkout a book 4. Return a book ")
        ...
        if b['available'] is True:
            for book in library_books:
                if book.get('id') == checkout_Id:
                    ...
                    book['due_date'] = datetime.now # put in date 2 weeks from now
                    book['checkouts'] += 1
...
def overdue(library_books):
    if library_books["due_date"] > datetime.now:
        print("")
